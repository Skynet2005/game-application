// src/components/cyber-rant/lib/actions/user.actions.ts
'use server';
...
export async function fetchUsers({ userId, searchString = '', pageNumber = 1, pageSize = 20, sortBy = 'desc', }: {
  ...
  try {
    ...
    const query: FilterQuery<typeof User> = { id: { $ne: userId } }; // Exclude the current user from the results.
...
export async function fetchUserCommunities(userId: string) {
  try {
    connectToDB();
    const userWithCommunities = await User.findOne({ id: userId })
      .populate({
        path: 'communities',
        model: Community,
    ...
    return userWithCommunities.communities;
  } catch (error: any) {
