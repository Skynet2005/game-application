...
export const useChatScroll = ({ chatRef, bottomRef, shouldLoadMore, loadMore, count, }: ChatScrollProps) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => { const scrollTop = topDiv?.scrollTop; if (scrollTop === 0 && shouldLoadMore) { loadMore() } };
  ...
  useEffect(() => {
    ...
    const shouldAutoScroll = () => {
      if (!hasInitialized && bottomDiv) { setHasInitialized(true); return true; }
